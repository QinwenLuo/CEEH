...
TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE:-500}
VAL_BATCH_SIZE=${VAL_BATCH_SIZE:-512}
MAX_PROMPT_LENGTH=${MAX_PROMPT_LENGTH:-2048}
MAX_RESPONSE_LENGTH=${MAX_RESPONSE_LENGTH:-20480}
OVERLONG_BUFFER_LENGTH=${OVERLONG_BUFFER_LENGTH:-4096}
OVERLONG_PENALTY_FACTOR=${OVERLONG_PENALTY_FACTOR:-0.5}
SAVE_FREQ=${SAVE_FREQ:-10}
TEST_FREQ=${TEST_FREQ:--1}
EPOCHS=${EPOCHS:-10}
GPUS=${GPUS:-$(nvidia-smi -L | wc -l)}
PROJECT=${PROJECT:-CEEH}
EXP=${EXP:-entropy-advantage}
LENGTH_REWARD_SCALE=${LENGTH_REWARD_SCALE:-0.1}
CHECKPOINT_DIR=${CHECKPOINT_DIR:-./checkpoints/${PROJECT}/${EXP}}
LOG_DIR=${LOG_DIR:-./logs}

MAX_LEN=$((MAX_PROMPT_LENGTH + MAX_RESPONSE_LENGTH))
PPO_MAX_LEN=$((2 * MAX_LEN))

if [ -z "$MODEL_PATH" ] || [ -z "$TRAIN_FILE" ] || [ -z "$TEST_FILE" ]; then
    echo "Please set MODEL_PATH, TRAIN_FILE, and TEST_FILE before running train.sh."
    exit 1
fi

mkdir -p "$LOG_DIR" "$CHECKPOINT_DIR"

PYTHONUNBUFFERED=1 python main_edlao.py \
    ...
    actor_rollout_ref.actor.retain_entropy_loss=False \
    ...
    length_rewards.reward_scale="$LENGTH_REWARD_SCALE" \
