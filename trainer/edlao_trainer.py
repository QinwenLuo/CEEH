...
class EDLAORayPPOTrainer(RayPPOTrainer):
    ...
    def __init__(
        ...
        self.ema_max = config.length_rewards.ema_max
        self.ema_min = config.length_rewards.ema_min
        self.length_records = np.zeros((len(train_dataset),))
        self.acc_records = np.zeros((len(train_dataset),)) - 1
    ...
    def compute_length_reward(self, index_list, rewards, data, current_acc): 
        ...
        length_reward_tensor = torch.zeros(data.batch['responses'].size(0), dtype=torch.float32)

        from collections import defaultdict
        from concurrent.futures import ThreadPoolExecutor

        def process_item(args):
            i, index, data_item = args

            prompt_ids = data_item.batch['prompts']
            prompt_length = prompt_ids.shape[-1]
            valid_response_length = data_item.batch['attention_mask'][prompt_length:].sum().item()

            if current_acc[i] < self.acc_records[index]:
                return i, 0, valid_response_length, index

            old_optimal_length = self.length_records[index]
            length_reward = 0.0

            if old_optimal_length != 0:
                length_reward = (old_optimal_length - valid_response_length) / old_optimal_length
            else:
                length_reward = 0.0

            return i, length_reward, valid_response_length, index
        
        args = []
        length_reward_nums = 0
        for i, (data_item, idx) in enumerate(zip(data, index_list)):
            if rewards[i].max() >= 1:
                args.append((i, idx, data_item))
        ...
        for i, length_reward, valid_response_length, index in results:
            length_reward_item = max(-0.9, length_reward * self.reward_scale)
            all_length_reward_tensor[i, valid_response_length - 1] = length_reward_item
            length_reward_tensor[i] = length_reward_item
            update_candidates[index].append(valid_response_length)
                
        for index, new_lens in update_candidates.items():
            old_min_len = self.length_records[index]
            new_min_len = np.min(np.array(new_lens)) 
            if old_min_len == 0:
                self.length_records[index] = new_min_len
            else:
                alpha = 0.2 if new_min_len < old_min_len else 0.05
                self.length_records[index] = alpha * new_min_len + (1 - alpha) * old_min_len

        return all_length_reward_tensor, length_reward_tensor, length_reward_nums
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        ratio = (difficulties == 1).float().mean()

                        metrics['training/difficulty_ratio'] = ratio.detach().item()
                        metrics['training/total_acc_mean'] = self.acc_records.mean().item()
                        metrics['training/total_acc_std'] = self.acc_records.std().item()
                        ...
                        if self.use_length_reward:
                            length_reward_tensor, length_reward_log, length_reward_nums = self.compute_length_reward(
                            ...
                            length_reward_metrics = {
                                ...
                                "perf/avg_target_length": self.length_records.mean(),
                            }
                        ...
                        for i, idx in enumerate(batch_dict["index"]):
                            ...
                            if self.acc_records[idx] <= 0:
                                ...
                                alpha = self.ema_max if new > old else self.ema_min
    ...
    def _save_checkpoint(self):
        ...
        np.save(
            os.path.join(self.config.trainer.default_local_dir, "acc_records.npy"),
            self.acc_records,
        )
    ...
    def _load_checkpoint(self):
        ...
        length_path = os.path.join(self.config.trainer.default_local_dir, "length_records.npy")
        acc_path = os.path.join(self.config.trainer.default_local_dir, "acc_records.npy")

        if self.global_steps > 0:
            assert os.path.exists(length_path
            ), f"Length records file not found at {length_path}"
            assert os.path.exists(acc_path
            ), f"Acc records file not found at {acc_path}"
            ...
            self.acc_records = np.load(
                os.path.join(
                    self.config.trainer.default_local_dir, "acc_records.npy"
                ),
                allow_pickle=True,
            )
            print(
                f"Acc records loaded from {os.path.join(self.config.trainer.default_local_dir, 'acc_records.npy')}"
            )
