...
class EDLAODataParallelPPOActor(DataParallelPPOActor):
    def __init__(
        ...
        if self.config.entropy_ablation:
            self.config.entropy_coeff = 0.0
            self.calculate_entropy = False
            self.config.use_entropy_advantage = False
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if self.calculate_entropy:
                        ...
                        elif self.config.entropy_coeff_annealing == "constant":
                            scale = 1.0

                    metrics["coef_scale"] = scale
                    ...
                    if self.config.use_entropy_advantage:
                        ...
                        advantages = advantages + (
                            entropy_adv * scale * difficulty_mask
                        )
                    ...
                    if entropy_coeff != 0:
                        retain_entropy_loss = self.config.get(
                            "retain_entropy_loss", False
                        )

                        if retain_entropy_loss:
                            hard_weight = 1 + (
                                self.config.entropy_factor - 1
                            ) * scale
                            coeff_tensor = 1 + difficulty_mask * (hard_weight - 1)
                            ...
                            entropy_loss_scale = scale

                        entropy_loss = agg_loss(
                            loss_mat=entropy,
                            loss_mask=response_mask * coeff_tensor,
                            loss_agg_mode=loss_agg_mode,
                        )
                        ...
                        policy_loss = (
                            pg_loss
                            - entropy_loss * entropy_coeff * entropy_loss_scale
                        )
                    else:
                        policy_loss = pg_loss
